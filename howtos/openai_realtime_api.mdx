...
sequenceDiagram
    ...
    OpenAI-->>DevBackend: Return client secret (sess_xxx)
...
- Other session parameters - See [OpenAI's session configuration reference](https://platform.openai.com/docs/api-reference/realtime-sessions)
...
- [Session Update Events](https://platform.openai.com/docs/api-reference/realtime-client-events/session/update) - Dynamically update session configuration
...
curl -X POST https://api.avaturn.live/api/v1/sessions \
  ...
  -d '{
    "conversation_engine": {
      "type": "openai-realtime",
      "client_secret": "sess_xxxxxxxxxxxx"
    }
  }'
```
