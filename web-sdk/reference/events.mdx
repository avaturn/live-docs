...
- **`local_audio_change`**  
  Triggered when the user turns their microphone on or off.  
  **Callback data**:  
  ```javascript
    localAudio: boolean  // true - microphone is active, false - microphone is muted
  ```

- **`local_mic_state_change`**  
  Triggered when the user's microphone permission state changes.  
  **Callback data**:  
  ```javascript
    state: 'blocked' | 'off' | 'sendable' | 'loading' | 'interrupted' | 'playable';  // Current microphone state
  ```
---
